function mouseproc_TVD(imageprev,roiprev)
...
%   6/6/20:     Fixed bug: deleted regions of spectra overlapping with
%               integration bounds would shorten integration region. Also
%               included shifting of integration bounds if peaks were
%               aligned by shifting.
%   6/9/20:     Removed integration bound shifting w/ peak alignment:
%               aligning should bring them all into same region!
%   6/10/20:    Fixed bug: apodization is affected by spectral deletions
%               b/c spectral width changes, so dt also changes! Fixed this
%               in generating time vector for apodization, etc. Also
%               improved bug fix for 6/6/20: rather than increasing the
%               integration bounds when there was a deletion, the deletion
%               region is now shortened in order to preserve the full
%               integration bounds.
%   6/18/20:    Modified so that deletion regions CANNOT delete any points
%               within integral bounds (obviated 6/6/20 fix)
%   6/30/20:    Edited apodization to properly apodize with deletions vs
%               w/o. Discovered that increasing origapod increases pH bias
%               with deletions adjacent to integral regions (up to 0.03 or 
%               so), likely b/c different magnitudes are now adjacent after
%               deletion
%
...
if BiCppm(1) < BiCppm(2) %flip around values so larger ppm value is 1st
    BiCppm = flip(BiCppm);
end
...
% DK debugging changes 6/10/20: to compare w/ various stages of denoising        
    images.CSI.imgdenoised = images.CSI.img;
%     images.CSI.img = images.CSI.imgpartdenoised;
%
    procCSI({num2str(newnp(1)) , num2str(newnp(2)) , ...
...
for jj = 1:length(spnames)
    spn = spnames{jj};
%     if alignflg && denoisedflg %adjust spectral bins by shiftmap -
%     REJECTED CHANGE b/c aligning should bring all peaks into the region
%         bin = zeros([size(images.shiftmap) length(images.spbin.(binname).(spn))]);
%         maps.(spn).(nname).img = zeros([images.CSI.npx(1) images.CSI.npx(2)...
%             length(images.spbin.(binname).(spn))]);
%         zf = size(images.CSI.imgdenoised,1) / size(images.shiftmap,1); 
%             %zerofilling factor
%         for kk = 1:size(images.shiftmap,1)
%             for ll = 1:size(images.shiftmap,2)
%                 bin(ll,kk,:) = images.spbin.(binname).(spn) - ...
%                     images.shiftmap(ll,kk);
%             end
%         end
%         bin = imresize(bin,zf); %resize to match image size
%         bin = round(bin); %so they are all integer values        
%         for kk = 1:images.CSI.npx(1)
%             for ll = 1:images.CSI.npx(2)
%                 maps.(spn).(nname).img(ll,kk,:) = abs(...
%                     images.CSI.(nname)(ll,kk,bin(ll,kk,:)));
%             end
%         end        
%     else
    ...
    while ~finishflg
        try
            ...
            bins.delete = [bins.delete find(images.ppm > nb(1) & ...
                images.ppm < (nb(1) + nb(3)) & ~(images.ppm < BiCppm(1) & ...
                images.ppm > BiCppm(2)) & ~(images.ppm < CO2ppm(1) & ...
                images.ppm > CO2ppm(2)))]; %mod to ensure that points 
                %within integral region are not deleted
        catch
        end
    end
...
if denoiseflg %perform 1st denoising
    sLR = sizeLR;
    sLR(1) = round(size(ksp.img,3) / 4 *7/8); %use 7/8 spectral SVs for 
        %first denoise (NOTE: noticed on MS494 dataset a SIGNIFICANT pH  
        %bias with 3/4 spectral SVs [0.05 unit]...)
    temp = tensorDenoising(temp,sLR); %1st denoise
end
temp = ifftn(temp); %back to FID
t(1,1,:) = linspace(0,ksp.ddf.dwelltime/1000*(ksp.ddf.specpoints-1),...
    size(temp,3)); %time vector for apodization/shifting 
...
if denoiseflg
% Perform 2nd and 3rd denoising before zerofilling, if denoiseflg = true
%     %DK edits 6/10/20: troubleshooting denoising at various steps
%     pad = (str2double(parsin{1}) - ksp.ddf.npix(1)) / 2;
%     temp2 = padarray(temp,[1 1 0]*pad); %zerofill spatially
%     pad = (str2double(parsin{2}) - ksp.ddf.specpoints);
%     temp2 = padarray(temp2,[0 0 1]*pad,0,'post'); %zerofill spectrally
%     images.CSI.imgdenoised = fftn(temp2); %to spectra
% %     images.spbin.img = images.spbin.imgdenoised; %so correct bins w/ deletions are used
%     %end DK edits 6/10/20     
    temp = tensorDenoising(temp,sizeLR); %2nd denoise; do this on FIDs 
    %rather than spectra b/c for high values of apodization the
    %de-apodization blows up after FT + iFT    
    % 3rd denoise: de-apodize, then denoise to remove residual near-Gaussian
    % noise
    deapdmat = 1 ./ apdmat;   
    temp = temp .* deapdmat;   
    temp = fftn(temp); %back to spectra
    temp = tensorDenoising(temp,sizeLR); %3rd denoise
...
%         ksp.ddf.npix(2),1); %gaussian     
    reapdmat = repmat(exp(-t*images.CSI.origapod*pi),ksp.ddf.npix(1), ...
        ksp.ddf.npix(2),1); %exponential
    temp = temp .* reapdmat;
end
%zerofilling
